...
export function WordDuelResults({ roundId, sessionId, score, onExit, onShowRipple }: WordDuelResultsProps) {
  ...
  useEffect(() => {
    async function fetchProof() {
      try {
        ...
        console.error("WordDuelResults: Proof fetch failed:", err);
      } finally {
    ...
    async function checkStatus() {
      ...
      try {
        const data = await publicClient.readContract({
          ...
          abi: [{ name: "getWordDuelRound", type: "function", stateMutability: "view",
            inputs: [{ name: "duelId", type: "uint256" }],
            outputs: [
              { name: "creator", type: "address" }, { name: "token", type: "address" },
              { name: "entryFee", type: "uint256" }, { name: "startTime", type: "uint64" },
              { name: "endTime", type: "uint64" }, { name: "topScorer", type: "address" },
              { name: "topScore", type: "uint256" }, { name: "poolBalance", type: "uint256" },
              { name: "playerCount", type: "uint256" }, { name: "finalized", type: "bool" },
            ],
          }],
        ...
        setRoundEnded(Number(data[4]) * 1000 <= Date.now());
        setPayoutFinalized(data[9] as boolean);
      } catch {}
    }
    checkStatus();
  ...
  const handleShare = async () => {
    const text = `I scored ${score} points in Word Duel on NimArena! 🏆 #NimArena #Cycle1`;
  ...
  return (
    <div className="pb-24 px-5 w-full max-w-md sm:max-w-lg md:max-w-xl mx-auto pt-6 text-center page-fade-in">
      ...
      {/* Trophy badge */}
      <div className="relative inline-flex items-center justify-center w-20 h-20 rounded-full bg-[#7C3AED]/15 border border-[#7C3AED]/30 text-[#A78BFA] mb-5">
