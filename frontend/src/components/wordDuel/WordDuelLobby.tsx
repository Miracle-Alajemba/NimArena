...
export function WordDuelLobby({ onStartWordDuel, onStartPractice }: WordDuelLobbyProps) {
        ...
        for (const r of data) {
          try {
            const entered = await publicClient.readContract({
              ...
              functionName: "wordDuelEntered",
              args: [BigInt(r.roundId), walletAddress],
            } as any);
            enteredStates[r.roundId] = entered as boolean;
          } catch (e) {
            console.error("WordDuelLobby: Failed to check entered status:", e);
          }
        }
        setEnteredRounds(enteredStates);
      }
    } catch (err) {
      console.error("WordDuelLobby: Failed to fetch rounds:", err);
      // Mock rounds fallback for preview
      setRounds([
        {
          roundId: 1,
          creator: "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC",
          entryFee: selectedCurrency === "USDT" ? "1000000" : "50000000000000000000",
          ...
          topScore: 24,
          poolBalance: selectedCurrency === "USDT" ? "5000000" : "250000000000000000000",
          playerCount: 5,
          finalized: false,
        },
      ]);
  ...
  const handleEnterRound = async (roundId: number, fee: string, tokenAddr: `0x${string}`) => {
    try {
      const hash = await enterWordDuel(roundId, fee, tokenAddr);
      if (hash) {
      ...
      console.error("WordDuelLobby: Enter round failed:", error);
    }
  };
  ...
  const handleCreateRound = async () => {
    try {
      ...
      console.error("WordDuelLobby: Create round failed:", error);
    }
  };
  ...
  return (
    <div className="pb-24 px-5 w-full max-w-md sm:max-w-lg md:max-w-xl mx-auto pt-4">
      ...
      <div className="flex flex-col gap-4">
        {loading && rounds.filter((r: any) => !r.token || r.token.toLowerCase() === tokenAddress.toLowerCase()).length === 0 ? (
        ...
        ) : rounds.filter((r: any) => !r.token || r.token.toLowerCase() === tokenAddress.toLowerCase()).length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-gray-500 gap-3 border border-[#1F1F2E] rounded-2xl bg-[#13131A]">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-center px-4">
              No active Word Duel rounds.<br />Create one to start playing!
            </span>
          </div>
        ) : (
          rounds.filter((r: any) => !r.token || r.token.toLowerCase() === tokenAddress.toLowerCase()).map((round: any) => {
            ...
            return (
              ...
              >
                ...
                {timeLeftSec === 0 ? (
                  ...
                  <button
                    onClick={() => onStartWordDuel(round.roundId, round.entryFee)}
                    style={{ minHeight: "44px" }}
                  ...
                  >
                    <Play className="w-4 h-4 fill-white" /> Play Word Duel
                  </button>
                ) : (
                  <button
          ...
          <div className="w-full max-w-sm rounded-2xl bg-[#13131A] border border-[#2B2B3D] p-6 shadow-2xl flex flex-col gap-4">
            ...
            <div className="flex gap-3 border-t border-[#1F1F2E] pt-4 mt-2">
              ...
              <button
                ...
                disabled={txLoading}
                style={{ minHeight: "44px" }}
                className="flex-1 rounded-xl bg-[#7C3AED] hover:bg-[#A78BFA] text-white text-xs font-bold uppercase transition-colors flex items-center justify-center"
              >
                {txLoading ? (
                  <div className="w-4 h-4 border-2 border-t-transparent border-white rounded-full animate-spin" />
                ) : (
